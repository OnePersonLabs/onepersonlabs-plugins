...
function activationEvidence(text, skill) {
  ...
  return new RegExp(`\\b(?:using|invoking|loaded|applying|I(?:['’]d|\\s+would)\\s+use)\\s+(?:(?:the|requested|manual-only|explicitly\\s+(?:selected|requested))\\s+)*\\$?(?:[a-z0-9][a-z0-9-]*:)?${escaped}(?![a-z0-9_-])`, 'iu').test(plain)
}
