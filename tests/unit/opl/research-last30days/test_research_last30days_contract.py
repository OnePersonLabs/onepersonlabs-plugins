...
SKILL_ROOT = REPOSITORY_ROOT / "plugins" / "opl" / "skills" / "research-last30days"
SCRIPT = SKILL_ROOT / "scripts" / "research-last30days.py"
PYTHON_COMMAND = [sys.executable, "-B", "-X", "utf8"]


class ResearchLast30DaysNamespaceContractTest(unittest.TestCase):
    def test_skill_package_uses_research_last30days_identity(self) -> None:
        self.assertEqual(SKILL_ROOT.name, "research-last30days")
        skill_text = (SKILL_ROOT / "SKILL.md").read_text(encoding="utf-8")
        self.assertIn("name: research-last30days\n", skill_text)
    ...
    def test_skill_uses_codex_interaction_and_tool_agnostic_web_wording(self) -> None:
        ...
        self.assertIn("& $env:RESEARCH_LAST30DAYS_PYTHON -B -X utf8", skill_text)
        self.assertIn('"$RESEARCH_LAST30DAYS_PYTHON" -B -X utf8', skill_text)
    ...
    def test_runtime_defaults_are_isolated_from_upstream_skill(self) -> None:
        with tempfile.TemporaryDirectory() as temp_home:
            ...
            process_env.pop("RESEARCH_LAST30DAYS_CONFIG_DIR", None)
        ...
        self.assertEqual(
            payload["config_dir"],
            str(Path(temp_home) / ".config" / "research-last30days"),
        )
        self.assertEqual(payload["keychain_prefix"], "research-last30days-")
        self.assertEqual(payload["pass_prefix"], "research-last30days/")

    def test_project_config_uses_agents_namespace(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            project = Path(temp_dir) / "project"
            nested = project / "src"
            project_env = project / ".agents" / "research-last30days.env"
            project_env.parent.mkdir(parents=True)
            project_env.write_text("RESEARCH_LAST30DAYS_DEFAULT_SEARCH=reddit\n", encoding="utf-8")
            (project / ".git").mkdir()
            nested.mkdir()
            ...
            command = [
                *PYTHON_COMMAND,
                ...
                (
                    ...
                    "'default_search': config.get('RESEARCH_LAST30DAYS_DEFAULT_SEARCH')"
                    "}))"
                ),
            ]
            process_env = os.environ.copy()
            ...
            process_env["RESEARCH_LAST30DAYS_CONFIG_DIR"] = ""
            process_env["RESEARCH_LAST30DAYS_TRUST_PROJECT_CONFIG"] = "1"
            result = subprocess.run(
                command,
                cwd=nested,
                env=process_env,
    ...
    def test_cli_help_uses_research_last30days_command_name(self) -> None:
        result = subprocess.run(
        ...
        self.assertIn("research-last30days", result.stdout)
    ...
    def test_version_resolution_is_anchored_to_nearest_codex_plugin_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            script_path = plugin_root / "skills" / "research-last30days" / "scripts" / "tool.py"
    ...
    def test_doctor_cache_is_explicit_cached_and_secret_free(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            process_env = {
                ...
                "RESEARCH_LAST30DAYS_CONFIG_DIR": temp_dir,
    ...
    def test_mock_saved_report_has_evidence_without_agent_scaffolding(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            process_env = {
                ...
                "HOME": temp_dir,
                "USERPROFILE": temp_dir,
                "RESEARCH_LAST30DAYS_CONFIG_DIR": str(config_dir),
            }
            result = subprocess.run(
                [
                    *PYTHON_COMMAND,
    ...
    def test_removed_surfaces_and_old_namespaces_do_not_return(self) -> None:
        ...
        self.assertIsNone(re.search(r"\.config/last30days", text))
