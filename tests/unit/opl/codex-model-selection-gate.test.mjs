...
test('rejects a direct exec with either explicit setting missing', () => {
  ...
  assert.equal(missing('codex exec -m gpt-5.6-luna "inspect this"'), 'model_reasoning_effort')
  ...
  assert.equal(missing('codex exec -m gpt-5.6-sol -c model_reasoning_effort= "inspect this"'), 'model_reasoning_effort')
...
test('accepts both explicit settings through flags and config overrides', () => {
  for (const command of [
    'codex exec -m gpt-5.6-luna -c model_reasoning_effort="low" "inspect this"',
    'codex exec --model=gpt-5.6-sol --config=model_reasoning_effort=high "implement this"',
    'codex exec -c model=gpt-6-astra -c model_reasoning_effort=medium "review this"',
    'codex -m gpt-5.6-sol exec -c model_reasoning_effort=medium "implement this"',
    'codex exec fork -m gpt-5.6-sol -c model_reasoning_effort=xhigh 01a094a7-0cbc-72a0-9935-9e3f2b7f0169 "continue"',
    'codex exec resume --model gpt-6-astra -c model_reasoning_effort=high --last "continue"',
    'codex exec review -m gpt-5.6-sol -c model_reasoning_effort=high --uncommitted',
    '& "C:\\Program Files\\Codex\\codex.exe" exec -m gpt-5.6-luna -c "model_reasoning_effort=high" "check"',
...
test('prompt text cannot impersonate CLI settings', () => {
  assert.match(reason('codex exec "Discuss --model=gpt-5.6-luna and -c model_reasoning_effort=low"'), /model.*effort/iu)
  assert.match(reason('codex exec -m gpt-5.6-luna write a guide to -c model_reasoning_effort=low'), /effort/iu)
})
...
test('recognizes a direct exec through the env wrapper', () => {
  ...
  assert.deepEqual(check('env FOO=bar codex exec -m gpt-5.6-sol -c model_reasoning_effort=medium inspect'), {})
})
