...
Run `./scripts/install.sh` to register this local marketplace and reinstall every plugin listed in `.agents/plugins/marketplace.json`. The script requires the `codex` CLI and `jq`. Set `CODEX_BIN` to use a different Codex executable.
