...
Run `./plugins/opl/scripts/reinstall-my-plugins.py` to register this local marketplace and reinstall every plugin listed in `.agents/plugins/marketplace.json`. The script requires Python 3 and the `codex` CLI. Set `CODEX_BIN` to use a different Codex executable.
