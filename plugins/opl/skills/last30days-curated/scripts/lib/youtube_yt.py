...
def is_ytdlp_installed() -> bool:
    ...
    When LAST30DAYS_CURATED_YOUTUBE_SSH_HOST is set, returns True without a local check --
...
def _ytdlp_ssh_host() -> Optional[str]:
    ...
    Set LAST30DAYS_CURATED_YOUTUBE_SSH_HOST=<ssh-alias> (e.g. 'macmini') in the environment
    ...
    To use a value from ~/.config/last30days-curated/.env, export it into the
    environment before invoking the engine, e.g. in a wrapper:
        set -a; source ~/.config/last30days-curated/.env; set +a
        python3 last30days-curated.py "..."
    """
    host = os.environ.get("LAST30DAYS_CURATED_YOUTUBE_SSH_HOST", "").strip()
    if not host:
        return None
    if not _SSH_HOST_ALIAS_RE.match(host):
        sys.stderr.write(
            f"[youtube_yt] WARNING: LAST30DAYS_CURATED_YOUTUBE_SSH_HOST={host!r} "
...
def _wrap_ytdlp_cmd(cmd: List[str]) -> List[str]:
    ...
    LAST30DAYS_CURATED_YOUTUBE_SSH_HOST were ever set to a value starting with `-`.
    """
...
def _ytdlp_sub_langs() -> str:
    """Caption languages to try, from LAST30DAYS_CURATED_YT_SUB_LANGS (default en,es,pt)."""
    raw = os.environ.get("LAST30DAYS_CURATED_YT_SUB_LANGS", "").strip()
    if not raw:
        return "en,es,pt"
