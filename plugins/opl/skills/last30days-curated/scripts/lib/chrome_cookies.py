...
# Other Chromium-based browsers, keyed by curated browser-selection name. Each maps to
# (profile base directory, macOS Keychain service name). Chrome and Brave keep
# their dedicated helpers below; everything here is
        ...
        for name, value, encrypted_value in cursor.fetchall():
            ...
            if encrypted_value and encrypted_value[:3] == b"v10":
                if not key_fetched:
                    ...
                    # cookie, which matters when scanning several configured browsers.
...
def extract_chrome_cookies_macos(domain: str, cookie_names: list[str]) -> Optional[dict[str, str]]:
    ...
    same ``Default/Network/Cookies`` and ``Default/Cookies`` probing as the
    rest of the Chromium family.
    """
...
def _profile_cookie_db(profile_dir: Path) -> Optional[Path]:
    ...
    Prefers ``Network/Cookies`` and then checks the flat ``Cookies`` location.
    Different supported browsers use both layouts.
    """
...
def _find_chromium_cookies_db(base_dir: Path) -> Optional[Path]:
    ...
    Each location is probed for both supported cookie paths. Chromium browsers create extra
