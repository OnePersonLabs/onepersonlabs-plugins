...
# for example, "Acme Studio use cases" yields primary_entity="acme studio"
# rather than "acme studio use cases". Kept in sync with
...
def _candidate_haystack(candidate: schema.Candidate) -> str:
    ...
    Includes transcript snippets, transcript highlights, and top-comment text
    so grounding uses every text surface visible in the report.
    """
...
#: the composite signal consumed by cluster scoring so engagement cannot erase
#: the entity-grounding penalty.
