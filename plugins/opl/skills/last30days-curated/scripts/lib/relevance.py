...
def token_overlap_relevance(
    ...
    Args:
        ...
        hashtags: Optional list of hashtags (TikTok). Concatenated
            hashtags are split to match query tokens (for example, "reactnative" matches "react").
    ...
    # Split concatenated hashtags (for example, "reactnative" matches "react", "native").
    if hashtags:
        for tag in hashtags:
            tag_lower = tag.lower()
            for qt in q_tokens:
