...
No API key is required for its public read-only endpoints.
"""
...
def _passes_any_informative_word(topic: str, event_title: str) -> bool:
    ...
    Validates merged comparison topics (for example, "Alpha vs Beta vs Gamma"),
    where a market mentioning just one of the entities
...
def parse_polymarket_response(response: Dict[str, Any], topic: str = "") -> List[Dict[str, Any]]:
    ...
    # returned only tangential matches (for example, "Acme best AI model"
