...
def ensure_supported_python(version_info: tuple[int, int, int] | object | None = None) -> None:
    ...
    sys.stderr.write(
        f"research-last30days requires Python {req}+.\n"
...
def resolve_requested_sources(args_search: str | None, config: dict) -> list[str] | None:
    ...
    RESEARCH_LAST30DAYS_DEFAULT_SEARCH config key (env var or .env file), then None
    ...
    default_search = (config.get("RESEARCH_LAST30DAYS_DEFAULT_SEARCH") or "").strip()
    if default_search:
        return parse_search_flag(default_search, flag_name="RESEARCH_LAST30DAYS_DEFAULT_SEARCH")
    return None


def slugify(value: str) -> str:
    slug = re.sub(r"[^a-z0-9]+", "-", value.lower()).strip("-")
    return slug or "research-last30days"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--no-browser-cookies", action="store_true",
                        help="Disable browser-cookie extraction even when RESEARCH_LAST30DAYS_FROM_BROWSER is configured")
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="N",
        help="Auto-discover N competitor entities and fan out research-last30days across all of them as a comparison (default N=2 -> 3-way: original + 2 peers; range 1..6). Use --competitors-list to override discovery.",
    )
    parser.add_argument(
        "--competitors-list",
...
def _render_save_and_print(
    ...
    if args.output:
        ...
        sys.stderr.write(f"[research-last30days] Saved output to {output_path}\n")
        sys.stderr.flush()
    if args.save_dir:
        ...
        sys.stderr.write(f"[research-last30days] Saved output to {save_path}\n")
        ...
        if entity_reports and len(entity_reports) > 1:
            for label, entity_report in entity_reports[1:]:
                ...
                sys.stderr.write(f"[research-last30days] Saved output to {peer_path}\n")
            peers_display = ", ".join(str(path) for path in comparison_peer_paths)
            sys.stderr.write(
                f"[research-last30days] Comparison artifact set: main={save_path}; "
...
def main() -> int:
    ...
    if args.debug:
        os.environ["RESEARCH_LAST30DAYS_DEBUG"] = "1"
    ...
    if args.save_dir is None:
        env_val = os.environ.get("RESEARCH_LAST30DAYS_MEMORY_DIR")
        args.save_dir = env_val if env_val is not None else config.get("RESEARCH_LAST30DAYS_MEMORY_DIR")
    ...
    if config.get("RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST") and "RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST" not in os.environ:
        os.environ["RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST"] = config["RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST"]
            ...
            if comp_explicit:
                ...
                if not resolve_mod._has_backend(config) and not args.mock:
                    sys.stderr.write(
                        ...
                        "  /research-last30days '{topic} vs {peer1} vs {peer2}' "
