...
Uses the engine's configured web path to discover relevant communities and
context before the planner runs.
"""
...
def _merge_category_peers(topic: str, subreddits: list[str]) -> tuple[list[str], Optional[str]]:
    """Extend the discovered subreddit list with category peers.
    ...
    every discovered sub (they are the freshest signal) and
    ...
    actually added (not when every peer was already discovered).
