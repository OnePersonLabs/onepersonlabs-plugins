...
def plan_query(
    *,
    ...
    ``internal_subrun`` distinguishes engine-created comparison runs from the
    initial user-requested run when selecting diagnostics.
    """
    ...
    if not internal_subrun:
        import sys
        print(
            "[Planner] No --plan was supplied; using deterministic planning. "
            "Pass --plan with the documented JSON schema for a tailored plan.",
            file=sys.stderr,
        )
...
- INTENT-MODIFIER HANDLING: when the topic contains one of {{use cases, use case, workflows, workflow, examples, tutorial, tutorials, review, reviews, comparison, applications, in practice, production, production use, how i use}}, strip that phrase from every search_query while keeping its meaning in ranking_query. Emit 4-5 paraphrased subqueries that express the intent as production, workflow or pipeline, review or experience, alternatives, and community discussion. Retrieve broadly and rank narrowly.
- DO NOT quote the user's full topic verbatim in search_query. Quote only multi-word proper nouns like "Acme Studio" or "React Native". Bare keywords retrieve more than exact-phrase searches.
...
def _fallback_plan(
    ...
    # Intent modifiers get paraphrased retrieval queries rather than a literal
    # phrase repeated across every source.
...
def _infer_intent(topic: str) -> str:
    ...
    if re.search(r"\b(trending|this week|right now|today|this month)\b", text):
        return "breaking_news"
    # Unknown topics use the balanced concept defaults instead of assuming news.
    return "concept"
...
def _default_source_weights(intent: str, sources: list[str]) -> dict[str, float]:
    ...
    if intent == "prediction":
        ...
        for source, bonus in {"youtube": 0.5}.items():
            if source in base:
                base[source] += bonus
    return base
...
def _keyword_query(topic: str, core: str) -> str:
    ...
    Quote only title-cased multi-word proper nouns ("Acme Studio",
    "React Native") so platform search engines preserve the
...
def _has_intent_modifier(topic: str) -> bool:
    """Return True when the topic contains a retrieval intent modifier."""
    text = topic.lower()
...
def _max_subqueries(intent: str, topic: str | None = None) -> int:
    # Comparison and intent-modifier topics need room for per-angle fanout.
