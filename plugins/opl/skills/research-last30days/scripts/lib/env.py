"""Environment and API key management for research-last30days skill."""
...
# Set RESEARCH_LAST30DAYS_CONFIG_DIR="" for clean/no-config mode
# Set RESEARCH_LAST30DAYS_CONFIG_DIR="/path/to/dir" for custom config location
_config_override = os.environ.get('RESEARCH_LAST30DAYS_CONFIG_DIR')
if _config_override == "":
    ...
    CONFIG_DIR = Path.home() / ".config" / "research-last30days"
    CONFIG_FILE = CONFIG_DIR / ".env"
...
# Example: `security add-generic-password -a "$USER" -s research-last30days-XAI_API_KEY -w "xai-..."`.
KEYCHAIN_SERVICE_PREFIX = "research-last30days-"
...
# Keychain's "research-last30days-<KEY>" service-name convention, so any user stores keys
# under one namespace without editing code. The prefix is resolved at call time
# (in get_config) from RESEARCH_LAST30DAYS_PASS_PREFIX in the process env or a config
# file, falling back to this default; included verbatim, so keep the trailing
# separator. Honors PASSWORD_STORE_DIR.
DEFAULT_PASS_PATH_PREFIX = "research-last30days/"
...
def _project_config_trusted(policy: ConfigLoadPolicy, file_env: dict[str, Any]) -> bool:
    if policy.allow_project_config:
        return True
    process_value = os.environ.get("RESEARCH_LAST30DAYS_TRUST_PROJECT_CONFIG")
    if process_value is not None:
        return _truthy(process_value)
    return _truthy(file_env.get("RESEARCH_LAST30DAYS_TRUST_PROJECT_CONFIG"))
...
def _check_file_permissions(path: Path) -> None:
    ...
    try:
        ...
        if mode & 0o044:
            sys.stderr.write(
                f"[research-last30days] WARNING: {path} is readable by other users. "
                f"Run: chmod 600 {path}\n"
            )
            sys.stderr.flush()
    except OSError as exc:
        sys.stderr.write(f"[research-last30days] WARNING: could not stat {path}: {exc}\n")
        sys.stderr.flush()
...
def _load_pass(keys: list[str], prefix: str) -> dict[str, str]:
    ...
    ``research-last30days-<key>`` service-name convention -- so any user stores keys under
    that namespace without editing code (prefix overridable via
    ``RESEARCH_LAST30DAYS_PASS_PREFIX``). The secret is decrypted in a subprocess and
...
def _find_project_env() -> Path | None:
    ...
    Searches for .agents/research-last30days.env in each parent directory,
    stopping at the git root, user's home directory, or filesystem root.
    """
    cwd = Path.cwd()
    for parent in [cwd, *cwd.parents]:
        candidate = parent / ".agents" / "research-last30days.env"
        if candidate.exists():
            return candidate
...
def get_config(policy: ConfigLoadPolicy | None = None) -> dict[str, Any]:
    ...
    Priority (highest wins):
      ...
      2. Trusted .agents/research-last30days.env (per-project config)
      3. ~/.config/research-last30days/.env (global config)
      4. macOS Keychain items prefixed ``research-last30days-`` (Darwin only)
      5. ``pass`` entries under ``research-last30days/`` (when available)
    """
    ...
    # (not import time, so a .env-set RESEARCH_LAST30DAYS_PASS_PREFIX is honored), and
    ...
    pass_prefix = (
        os.environ.get("RESEARCH_LAST30DAYS_PASS_PREFIX")
        or merged_env.get("RESEARCH_LAST30DAYS_PASS_PREFIX")
    ...
    keys = [
        ...
        ('RESEARCH_LAST30DAYS_REASONING_PROVIDER', 'auto'),
        ('RESEARCH_LAST30DAYS_PLANNER_MODEL', None),
        ('RESEARCH_LAST30DAYS_RERANK_MODEL', None),
        ('RESEARCH_LAST30DAYS_X_MODEL', None),
        ('RESEARCH_LAST30DAYS_X_BACKEND', None),
        ('RESEARCH_LAST30DAYS_REDDIT_BACKEND', None),
        # Doctor cache freshness window in seconds (doctor --cached).
        ('RESEARCH_LAST30DAYS_DOCTOR_TTL', None),
        ('RESEARCH_LAST30DAYS_REDDIT_SC_MIN_ITEMS', None),
        ('RESEARCH_LAST30DAYS_MEMORY_DIR', None),
        ...
        ('RESEARCH_LAST30DAYS_SEARXNG_URL', None),
        ('RESEARCH_LAST30DAYS_FROM_BROWSER', None),
        ('RESEARCH_LAST30DAYS_TRUST_PROJECT_CONFIG', None),
        ('RESEARCH_LAST30DAYS_INCLUDE_SOURCES', ''),
        ('RESEARCH_LAST30DAYS_EXCLUDE_SOURCES', ''),
        ('RESEARCH_LAST30DAYS_DEFAULT_SEARCH', ''),
        ('RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST', None),
        ('RESEARCH_LAST30DAYS_YT_SUB_LANGS', 'en,es,pt'),
    ]
...
def cookie_extraction_browsers(config: dict[str, Any]) -> list[str]:
    """Browsers to try for cookie extraction, honoring RESEARCH_LAST30DAYS_FROM_BROWSER.
    ...
    - ``RESEARCH_LAST30DAYS_FROM_BROWSER=<name>`` - one browser (for example, ``firefox``),
      ``edge``, ``arc``).
    - ``RESEARCH_LAST30DAYS_FROM_BROWSER=firefox,safari`` - an explicit browser list.
    - ``RESEARCH_LAST30DAYS_FROM_BROWSER=auto`` - also try every Chromium browser (user accepts the
      Keychain dialog when needed).
    - ``RESEARCH_LAST30DAYS_FROM_BROWSER=off`` - returns [] (extraction disabled).
    ...
    from_browser = (config.get("RESEARCH_LAST30DAYS_FROM_BROWSER") or "").strip().lower()
    if not from_browser:
        return []
    if from_browser == "off":
        return []
    ...
    if "," in from_browser:
        ...
        if unknown:
            sys.stderr.write(
                "[research-last30days] WARNING: RESEARCH_LAST30DAYS_FROM_BROWSER ignored unrecognized browser(s): "
    ...
    sys.stderr.write(
        f"[research-last30days] WARNING: RESEARCH_LAST30DAYS_FROM_BROWSER='{from_browser}' is not a recognized "
...
X_BACKEND_PIN_VAR = 'RESEARCH_LAST30DAYS_X_BACKEND'
REDDIT_BACKEND_PIN_VAR = 'RESEARCH_LAST30DAYS_REDDIT_BACKEND'
REDDIT_SC_MIN_ITEMS_VAR = 'RESEARCH_LAST30DAYS_REDDIT_SC_MIN_ITEMS'
...
def x_backend_chain(config: dict[str, Any], local_only: bool = False) -> list[str]:
    ...
    A ``RESEARCH_LAST30DAYS_X_BACKEND`` pin forces a single backend (no failover): the
...
def is_youtube_comments_available(config: dict[str, Any]) -> bool:
    ...
    ``RESEARCH_LAST30DAYS_INCLUDE_SOURCES``. Cost is
...
def is_tiktok_comments_available(config: dict[str, Any]) -> bool:
    ...
    Requires SCRAPECREATORS_API_KEY and tiktok_comments in RESEARCH_LAST30DAYS_INCLUDE_SOURCES.
...
def _parse_include_sources(config: dict[str, Any]) -> set[str]:
    ...
    raw = config.get('RESEARCH_LAST30DAYS_INCLUDE_SOURCES') or ''
