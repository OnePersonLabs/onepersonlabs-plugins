"""Xquik X search source for the Research Last 30 Days pipeline.
...
def expand_xquik_queries(topic: str, depth: str) -> List[str]:
    ...
    # Never let the core collapse to a single
...
def search_xquik(
    ...
    for query_text in queries:
        ...
        if auth_error:
            ...
            # and 402-unpaid surfaced by diagnostics) -- return it so the caller
...
def _execute_search(
    ...
    *,
    ...
    try:
        ...
        if status == 402:
            # An unpaid key is fatal for this source and must be visible to the
            # live caller rather than represented as an empty result set.
...
def search_handles(
    ...
    *,
    ...
    The topic is not AND'd into the query --
