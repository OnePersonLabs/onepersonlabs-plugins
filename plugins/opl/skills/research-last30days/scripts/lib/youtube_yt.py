...
transcript extraction. No API key is required for this lane.
"""
...
def is_ytdlp_installed() -> bool:
    ...
    When RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST is set, returns True without a local check --
...
def _ytdlp_ssh_host() -> Optional[str]:
    ...
    Set RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST=<ssh-alias> (e.g. 'macmini') in the environment
    ...
    To use a value from ~/.config/research-last30days/.env, export it into the
    environment before invoking the engine, e.g. in a wrapper:
        set -a; source ~/.config/research-last30days/.env; set +a
        python3 research-last30days.py "..."
    """
    host = os.environ.get("RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST", "").strip()
    if not host:
        return None
    if not _SSH_HOST_ALIAS_RE.match(host):
        sys.stderr.write(
            f"[youtube_yt] WARNING: RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST={host!r} "
...
def _wrap_ytdlp_cmd(cmd: List[str]) -> List[str]:
    ...
    RESEARCH_LAST30DAYS_YOUTUBE_SSH_HOST were ever set to a value starting with `-`.
    """
...
def _ytdlp_sub_langs() -> str:
    """Caption languages to try, from RESEARCH_LAST30DAYS_YT_SUB_LANGS (default en,es,pt)."""
    raw = os.environ.get("RESEARCH_LAST30DAYS_YT_SUB_LANGS", "").strip()
    if not raw:
        return "en,es,pt"
...
def fetch_transcript(
    ...
    Args:
        ...
        status: Optional dict mutated by the direct-HTTP path to record
            per-video signals like ``no_caption_tracks`` so callers can
            distinguish unavailable captions from retrieval failures.
...
def fetch_transcripts_parallel(
    ...
    Args:
        ...
        out_captions_disabled: Optional set mutated to record video_ids whose
            ...
            Callers may omit this output collector.
...
def search_and_transcribe(
    ...
    # Step 3: attach transcripts and preserve whether the uploader disabled
    # captions so synthesis can distinguish that state from retrieval failure.
    core_topic = _extract_core_subject(topic)
    for item in items:
        vid = item["video_id"]
...
def _fetch_video_comments(
    ...
    for c in raw_comments[:max_comments]:
        ...
        # ScrapeCreators may return an author object; fixtures may use a string.
        author = c.get("author") or c.get("author_name", "")
        if isinstance(author, dict):
            author = author.get("name") or author.get("handle") or ""

        # Accept both engagement.likes and top-level likes.
