...
FLOOR of the search-source ladder:
    ...
    paid engine backend  >  keyless engine search

It never preempts a configured provider; the dispatcher uses this module only
as its no-credential fallback.

Two vendor-neutral rungs, both stdlib-only via :mod:`http`:
  1. DuckDuckGo HTML endpoint (no key, no instance to maintain).
  2. A configurable SearXNG instance returning JSON (``RESEARCH_LAST30DAYS_SEARXNG_URL``),
...
def keyless_search(
    ...
    used = "ddg"
    if not items:
        searxng_url = (config.get("RESEARCH_LAST30DAYS_SEARXNG_URL") or "").strip()
        if searxng_url:
