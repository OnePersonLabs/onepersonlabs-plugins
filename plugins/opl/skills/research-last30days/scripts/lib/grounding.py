...
def _reddit_excluded(config: dict) -> bool:
    """Return True when RESEARCH_LAST30DAYS_EXCLUDE_SOURCES contains Reddit.

    Respects the same suppression knob the pipeline uses for source gating,
    so excluded Reddit content is not
    smuggled back in via web-search URLs.
    """
    raw = (config.get("RESEARCH_LAST30DAYS_EXCLUDE_SOURCES") or "").split(",")
...
def _enrich_reddit_items(items: list[dict]) -> list[dict]:
    ...
    This fetches Reddit results through the public JSON API so the dedicated
    Reddit adapter can normalize thread bodies and comments.

    Callers should gate this with curated Reddit exclusion handling (see
