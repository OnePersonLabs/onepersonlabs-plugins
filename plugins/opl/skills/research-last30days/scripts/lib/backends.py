"""Backend-chain descriptors with predicted selection for doctor.
...
call or credential spend. Binary-backed lanes use
``health.probe_dependency`` so an unexecutable shim reads as broken rather
than available.

This module observes and predicts only; it never changes runtime routing.
"""
...
# Rollup tiers for a resolved chain.
TIER_OK = "ok"
TIER_WARN = "warn"
TIER_ERROR = "error"
...
# Chain fixes embed the registry's CLI forms so backend findings and doctor
# prescribe the same command for a failure condition.
_SC_PRESCRIPTION = (
    "set SCRAPECREATORS_API_KEY in the curated config: "
    f"{prescriptions.get('scrapecreators', 'key_missing').fix_cli}"
)
_X_COOKIES_PRESCRIPTION = (
    "configure an explicitly authorized browser: "
...
def _key_probe(name: str, key_var: str, requires: str, note: str = "") -> Callable:
    ...
    def probe(config: Dict[str, Any]) -> BackendFinding:
        ...
        prescription = note or f"set {key_var} in ~/.config/research-last30days/.env"
        return BackendFinding(
            name=name,
...
def _probe_bird(config: Dict[str, Any]) -> BackendFinding:
    ...
    if not bird_x.is_bird_installed():
        ...
        return BackendFinding(
            ...
            prescription="reinstall the owning Codex plugin to restore lib/vendor/bird-search",
            requires=requires,
        )
    node = health.probe_dependency("node")
    if node.status != health.OK:
...
def _probe_ytdlp(config: Dict[str, Any]) -> BackendFinding:
    """Probe yt-dlp as missing, broken, timed out, or ready."""
    ...
    return BackendFinding(
        ...
        requires="yt-dlp on the Codex subprocess PATH",
    )
