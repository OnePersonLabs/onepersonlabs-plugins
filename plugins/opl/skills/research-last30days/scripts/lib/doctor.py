"""Current-state diagnostics for Research Last 30 Days."""
...
CACHE_SCHEMA = "research-last30days-doctor-cache"
...
def _runtime_block() -> dict[str, Any]:
    return {
        ...
        "engine": str((Path(__file__).resolve().parents[1] / "research-last30days.py")),
...
def _cache_ttl(config: dict[str, Any]) -> int:
    raw = os.environ.get("RESEARCH_LAST30DAYS_DOCTOR_TTL")
    if raw is None:
        raw = config.get("RESEARCH_LAST30DAYS_DOCTOR_TTL")
    try:
...
def _fingerprint(config: dict[str, Any]) -> str:
    signals = {
        ...
        "include_sources": str(config.get("RESEARCH_LAST30DAYS_INCLUDE_SOURCES") or ""),
        "exclude_sources": str(config.get("RESEARCH_LAST30DAYS_EXCLUDE_SOURCES") or ""),
    }
...
def _write_cache(config: dict[str, Any], report: dict[str, Any]) -> None:
    ...
    try:
        ...
        sys.stderr.write(f"[research-last30days] doctor cache write failed: {exc}\n")
