"""Bird X search client for the Research Last 30 Days pipeline.
...
def get_bird_status() -> Dict[str, Any]:
    ...
    Returns:
        Dict with keys: installed, authenticated, username
    """
...
def _invoke_bird_subprocess(query: str, count: int, timeout: int):
    ...
    def _register(pid: int) -> None:
        ...
        try:
            from .process_registry import register_child_pid
    ...
    try:
        ...
        if pid_holder:
            try:
                from .process_registry import unregister_child_pid
...
def search_handles(
    ...
    def _search_one_handle(handle: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return []
        items = parse_bird_response(response, query=core_topic)
        # Log both success and empty results so the handle lane is observable.
