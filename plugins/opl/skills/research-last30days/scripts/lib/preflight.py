...
structured REFUSE message. The caller (scripts/research-last30days.py main()) writes
...
Only the demographic-shopping class is implemented because it needs user
context before retrieval can produce useful recommendations.
"""
...
def _has_qualifier(topic: str) -> bool:
    ...
    These details make a demographic shopping query specific enough to run.
...
def check_low_signal_demographic_query(topic: str) -> str | None:
    """Return a clarification request for a low-signal shopping query.

    A literal phrase such as
...
def _refuse_message(topic: str) -> str:
    return (
        f'[research-last30days] The topic "{topic}" is too broad for useful recent-source retrieval.\n'
        "\n"
        "Please provide at least one interest or hobby, the relationship to the "
        "recipient, and a budget range.\n"
    )
