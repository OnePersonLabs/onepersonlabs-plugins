...
Use `ENGINE` and `RESEARCH_LAST30DAYS_PYTHON` resolved by `SKILL.md`.
...
```bash
"$RESEARCH_LAST30DAYS_PYTHON" "$ENGINE" --preflight
```
...
```bash
"$RESEARCH_LAST30DAYS_PYTHON" "$ENGINE" doctor --json
```
...
- Global: `~/.config/research-last30days/.env`
- Project: `.agents/research-last30days.env`, discovered from the working
  directory up to the repository root
- Project trust gate: `RESEARCH_LAST30DAYS_TRUST_PROJECT_CONFIG=1` must come
  from the process environment or global config; a project file cannot trust
  itself
- macOS Keychain services: `research-last30days-<KEY>`
- `pass` entries: `research-last30days/<KEY>` by default

Process environment values have highest priority, followed by a trusted
project file, global config, Keychain, and `pass`.

Never copy values from `~/.config/research-last30days`, `.agents/research-last30days.env`, or
...
the engine to read. Set `RESEARCH_LAST30DAYS_FROM_BROWSER` to that browser only;
...
```bash
mkdir -p "$HOME/.config/research-last30days"
chmod 700 "$HOME/.config/research-last30days"
touch "$HOME/.config/research-last30days/.env"
chmod 600 "$HOME/.config/research-last30days/.env"
```
