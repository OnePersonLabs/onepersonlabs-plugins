...
1. Resolve `repo_root` with `git rev-parse --show-toplevel` from the current
   working directory.
2. Require both `$repo_root/install.sh` and
...
Run `CODEX_BIN="$codex_bin" ./install.sh` from `repo_root` and retain its complete
output and exit status. The repository installer owns marketplace removal,
plugin uninstall, local-directory marketplace registration, reinstall, and the
initial hook-trust inspection; do not reproduce those operations separately.
