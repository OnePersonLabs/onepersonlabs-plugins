...
3. **Get apply instructions**
   ...
   Surface every returned `warnings` entry, even when state is `ready` or `all_done`. For missing delta specs, create the required specs or declare `skip_specs: true` in `.openspec.yaml` only when the change truly changes no specified behavior; never use it to hide missing work. Repair unread delta paths instead of ignoring them. Re-run instructions and validation after repair.
