...
```bash
node .agents/skills/openspec-x-orchestrate/scripts/opsxx-orchestrate.mjs --dry-launch --json
```
...
5. Run `$openspec-verify-change <change>`, `$adversarial-review`, feasible change-required smoke checks, and required validation. Record the successful gates:

```bash
node .agents/skills/openspec-x-orchestrate/scripts/opsxx-orchestrate.mjs --checkpoint-archive <change> --gate openspec-verify-change --gate adversarial-review --gate smoke --gate validate
```

6. Run unchanged `$openspec-archive-change <change>`, then run the exact resume command emitted by the checkpoint operation.
