...
import { buildChangeGraph } from "../../openspec-x-dependency-audit/scripts/opsxx-deps.mjs";
...
const packagedSkillsRoot = path.join(projectRoot, "opl-openspec", "skills");

function makeFixture(changes) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "openspec-orch-test-"));
...
test("only the worktree orchestrator package and canonical graph import remain", () => {
  assert.equal(
    fs.existsSync(
      path.join(
        ...
        ".agents",
        "skills",
        "openspec-x-orchestrate-worktrees",
      ),
    ),
    false,
  );
  assert.equal(
    fs.existsSync(
      path.join(
        ...
        ".codex",
        "agents",
        "openspec-x-orchestrate-worker.toml",
      ),
    ),
    false,
  );
  ...
  assert.deepEqual(
    ...
    ["../../openspec-x-dependency-audit/scripts/opsxx-deps.mjs"],
  );
  ...
  try {
    ...
    assert.ok(
      PARENT_INTEGRATION_STEPS.indexOf("validate") <
        PARENT_INTEGRATION_STEPS.indexOf("openspec-sync-specs"),
    ...
    assert.deepEqual(plan.authoringEvents, [
      {
        ...
        command: "$openspec-ff-change add-fast-path",
      },
    ]);
    ...
    assert.deepEqual(plan.authoringEvents, [
      {
        type: "author",
        change: "add-apply",
        command: "$openspec-apply-change add-apply",
      },
    ]);
    assert.deepEqual(plan.launches, []);
    assert.deepEqual(plan.queuedLaunches, []);
    ...
    assert.deepEqual(plan.authoringEvents, [
      {
        type: "author",
        change: "add-continue",
        command: "$openspec-continue-change add-continue",
      },
    ]);
    assert.deepEqual(plan.launches, []);
    assert.deepEqual(plan.queuedLaunches, []);
...
test("worktree worker prompt replaces the assigned change name", () => {
  ...
  assert.match(prompt, /\$openspec-apply-change add-example/);
  ...
  assert.doesNotMatch(prompt, /\$openspec-propose/);
  assert.doesNotMatch(prompt, /Run `\$openspec-verify-change/);
  assert.doesNotMatch(prompt, /Run `\$openspec-sync-specs/);
...
test("codex worker launch uses direct argv and cwd with unusual paths", () => {
  ...
  assert.match(launch.prompt, /\$openspec-apply-change add-example/);
  assert.equal(
    launch.summaryPath,
    "/tmp/codex home [one]/worktrees/.logs/add-example/summary.txt",
  );
...
test("dirty target inspection reports branch, dirty files, and distance from main head", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "openspec-dirty-target-"));
...
test("artifact comparison ignores only tasks.md checkbox differences", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-artifact-compare-"),
  );
...
test("dirty target decision warns on task direction and recommends restart only when completion matches", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-dirty-decision-"),
  );
  const mainDir = path.join(root, "main");
  const worktreeDir = path.join(root, "worktree");
  const change = "add-example";
  ...
  try {
    ...
    assert.equal(
      extraComparison.artifactsIdenticalIgnoringTaskCheckboxes,
      true,
    );
    assert.deepEqual(extraComparison.mainCompletedMissingInWorktree, []);
    assert.deepEqual(extraComparison.worktreeExtraCompleted, [
      "1.2 Extra task",
    ]);
...
test("detached worker state is restartable and exposes completion without a batch barrier", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "openspec-worker-state-"));
  const descriptor = buildWorkerDescriptor({
    change: "add-alpha",
    repoName: "openspec",
    env: { CODEX_HOME: path.join(root, "codex-home") },
...
test("detached launch returns immediately and releases the child handle", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-detached-launch-"),
  );
  const descriptor = buildWorkerDescriptor({
    change: "add-alpha",
    repoName: "openspec",
...
test("worker state collection preserves missing and malformed result evidence", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-worker-failure-"),
  );
...
test("worktree include copying copies matched files and no-ops without include file", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-worktree-includes-"),
  );
  const repo = path.join(root, "repo");
...
test("worktree include copying rejects patterns outside the repo root", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-worktree-includes-bad-"),
  );
  const repo = path.join(root, "repo");
  const worktree = path.join(root, "worktree");
  fs.mkdirSync(repo, { recursive: true });
...
test("retained worktree capacity fails closed without deleting evidence", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-worktree-prune-"),
  );
...
test("retained worktree capacity permits a free slot without mutation", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-worktree-prune-protected-"),
  );
  const worktreeRoot = path.join(root, "worktrees");
  fs.mkdirSync(worktreeRoot, { recursive: true });
  const now = Date.now() / 1000;
...
test("create worker worktree copies root include files after git worktree add", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-worker-create-includes-"),
  );
  const repo = path.join(root, "repo");
  const codexHome = path.join(root, "codex-home");
...
test("preflight reads branch and status from a real worker worktree", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-worker-preflight-"),
  );
  const repo = path.join(root, "repo");
  const codexHome = path.join(root, "codex-home");
  fs.mkdirSync(path.join(repo, "openspec", "changes", "add-example"), {
...
test("worker result merges into main and cleanup waits until after merge", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "openspec-worker-merge-"));
  const repo = path.join(root, "repo");
  const codexHome = path.join(root, "codex-home");
  fs.mkdirSync(path.join(repo, "openspec", "changes", "add-example"), {
    recursive: true,
...
test("archive checkpoint survives restart, rejects unrelated dirt, and commits exact lifecycle paths", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-archive-change-resume-"),
  );
  const repo = path.join(root, "repo");
...
test("checkpoint handling rejects dirty primary, corrupt state, and stale HEAD", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-checkpoint-failures-"),
  );
  const change = "validate-terminal";
  const changeDir = path.join(root, "openspec", "changes", change);
  const stateDir = path.join(root, ".temp", "openspec-x-orchestrate");
...
test("aggregate validation failure preserves committed checkpoints", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-finalize-failure-"),
  );
  const stateDir = path.join(root, ".temp", "openspec-x-orchestrate");
...
test("checkpoint requires every integrated gate and successful finalize cleans state", () => {
  const root = fs.mkdtempSync(
    path.join(os.tmpdir(), "openspec-finalize-success-"),
  );
  const change = "validate-terminal";
  const changeDir = path.join(root, "openspec", "changes", change);
  fs.mkdirSync(changeDir, { recursive: true });
