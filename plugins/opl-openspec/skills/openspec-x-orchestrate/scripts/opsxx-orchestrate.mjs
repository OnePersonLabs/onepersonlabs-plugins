#!/usr/bin/env node
// Deterministic scheduler substrate for openspec-x-orchestrate.
...
import {
  ...
  buildChangeGraph,
} from "../../openspec-x-dependency-audit/scripts/opsxx-deps.mjs";
...
export function runtimeStateRoot({ repoDir = repoRoot } = {}) {
  return path.join(repoDir, ".temp", "openspec-x-orchestrate");
}
...
export function compareChangeArtifacts({
  ...
  const worktreeExtraCompleted = setDifference(
    worktreeCompleted,
    mainCompleted,
  );
...
async function main() {
  try {
    ...
    if (args.integrateWorker) {
      ...
      console.log(
        `merged ${args.integrateWorker}; run $openspec-verify-change, $adversarial-review, smoke, and validation before --checkpoint-archive`,
      );
      return;
    }
    if (args.checkpointArchive) {
      ...
      console.log(
        `awaiting root archive for ${checkpoint.change}; after $openspec-archive-change, run: node .agents/skills/openspec-x-orchestrate/scripts/opsxx-orchestrate.mjs --resume-archive ${checkpoint.change}`,
      );
      return;
    }
