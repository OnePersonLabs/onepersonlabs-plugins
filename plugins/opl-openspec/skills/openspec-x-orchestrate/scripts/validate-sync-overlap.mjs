 ...
 * $openspec-sync-specs is agent-driven and merges individual requirement
 ...
 * co-claims) -- $openspec-x-orchestrate calls it on each candidate wave to keep
 * co-claiming changes in separate waves.
 *
 * Usage:
 *   node .agents/skills/openspec-x-orchestrate/scripts/validate-sync-overlap.mjs [--changes-root <dir>] [--json] [change ...]
 *
