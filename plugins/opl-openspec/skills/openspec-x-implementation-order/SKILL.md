---
name: "openspec-x-implementation-order"
...
Each active change's `## Dependencies` section (upstream edges only). **Do not re-implement the grammar in prose here** -- the one executable definition is the `$openspec-x-dependency-audit` skill's `scripts/opsxx-deps.mjs` (the same parser the dependency-gate hook, `$openspec-x-advance`, and `$openspec-x-orchestrate` use). Run it and render its JSON:

```bash
node .agents/skills/openspec-x-dependency-audit/scripts/opsxx-deps.mjs --graph
```
...
1. **Run `node .agents/skills/openspec-x-dependency-audit/scripts/opsxx-deps.mjs --graph`.** That JSON is the graph; you do not parse proposals yourself.
