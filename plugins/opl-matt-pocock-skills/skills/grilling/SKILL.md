---
name: grilling
description: Grill the user relentlessly about a plan, decision, or idea through structured question rounds when available. Use when the user wants to stress-test their thinking or uses any 'grill' trigger phrases.
---
...
Ask every decision through the available question interface. Use
structured questions when the host provides them. Otherwise ask a concise
question in the response and wait. Commentary may report evidence, assumptions,
and progress, but must not solicit answers.
...
For a question in an ordinary response, ask one concise decision question and
wait for the answer.
...
At the start of a round, freeze the current frontier. Ask that whole frozen frontier before recomputing it. When a structured question interface limits a call, page the frontier across consecutive calls without asking permission to continue. Preserve every settled answer while paging.
...
Then ask for a final shared-understanding check. Recommend confirmation only when the synthesis is complete; also offer an option to keep grilling. Confirmation settles the design discussion only. It does not authorize implementation, file changes, external actions, or any other mutation beyond authority the user separately granted.
