# OPL Matt Pocock Skills

Based on [Matt Pocock’s skills](https://github.com/mattpocock/skills).

This plugin combines the retained engineering, productivity, project setup, and
experimental skills in one install. Each skill keeps its invocation policy.

## Engineering

- [$ask-matt](skills/ask-matt/SKILL.md)
- [$code-review](skills/code-review/SKILL.md)
- [$diagnosing-bugs](skills/diagnosing-bugs/SKILL.md)
- [$implement](skills/implement/SKILL.md)
- [$prototype](skills/prototype/SKILL.md)
- [$research](skills/research/SKILL.md)
- [$resolving-merge-conflicts](skills/resolving-merge-conflicts/SKILL.md)
- [$setup-matt-pocock-skills](skills/setup-matt-pocock-skills/SKILL.md)
- [$to-spec](skills/to-spec/SKILL.md)
- [$to-tickets](skills/to-tickets/SKILL.md)
- [$triage](skills/triage/SKILL.md)
- [$wayfinder](skills/wayfinder/SKILL.md)

## Productivity

- [$grill-me](skills/grill-me/SKILL.md)
- [$grilling](skills/grilling/SKILL.md)

## Project setup

- [$migrate-to-shoehorn](skills/migrate-to-shoehorn/SKILL.md)
- [$setup-pre-commit](skills/setup-pre-commit/SKILL.md)

## Experimental

- [$loop-me](skills/loop-me/SKILL.md)
- [$setup-ts-deep-modules](skills/setup-ts-deep-modules/SKILL.md)
