...
A skill bundled with OPL Caveman that compresses project memory files
(`CLAUDE.md`, todos, preferences) into caveman format, reducing repeated input size.

Agents that reload project memory files pay repeated input tokens. Caveman
shortens supported natural-language files.
...
data directory     ← human-readable backup
```

The backup remains in the data directory rather than next to the live file, so
skill auto-loaders do not read it twice. Path is
`$XDG_DATA_HOME/caveman-compress/backups/<parent-dir-name>/` on macOS and Linux,
or `%LOCALAPPDATA%\caveman-compress\backups\<parent-dir-name>\` on Windows. To
restore or revise the original, copy the backup over the compressed source.
The compressor refuses to run again while that backup path exists; move or
rename the old backup before recompressing.
...
Compress is bundled with the OPL Caveman plugin. Install it, then use `/caveman-compress`.
...
write original   → out-of-tree backup data directory
```
...
A project memory file loaded on every session start adds repeated input tokens.
A 1,000-token file adds 1,000 input tokens per session, or 100,000 across 100
sessions.
...
- `caveman`: ask the agent to answer in shorter prose
